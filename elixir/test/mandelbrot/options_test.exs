  ...
  describe ".parse" do
		context "when specifying all options" do
			...
			it "parses the random seed" do
				expect parse(full_json).seed |> to_eq 12345
			end
			it "parses the upper-left corner" do
				expect parse(full_json).upper_left |> to_eq %Complex{ real: 0.0, imag: 55.2 }
			end
			it "parses the lower-right corder" do
				expect parse(full_json).lower_right |> to_eq %Complex{ real: 92.3, imag: 120.3 }
			end
			it "parses the c parameter" do
				expect parse(full_json).c |> to_eq %Complex{ real: 3.14, imag: 4.13 }
			end
			it "parses the z parameter" do
				expect parse(full_json).z |> to_eq %Complex{ real: 4.4,  imag: 1.1 }
			end
			it "parses the r parameter" do
				expect parse(full_json).r |> to_eq %Complex{ real: 9.9,  imag: 3.3 }
			end
			it "parses the p parameter" do
				expect parse(full_json).p |> to_eq %Complex{ real: 0.3,  imag: 0.5 }
			end
		end

		context "when relying on defaults" do
			it "still parses the fractal type" do
				expect parse(default_json).fractal |> to_eq :julia
			end
			...
			it "defaults the random seed" do
				expect parse(default_json).seed |> to_eq 666
			end
			it "still parses the upper-left corner" do
				expect parse(default_json).upper_left |> to_eq %Complex{ real: 5.0, imag: 6.0 }
			end
			it "still parses the lower-right corder" do
				expect parse(default_json).lower_right |> to_eq %Complex{ real: 6.0, imag: 5.0 }
			end
			it "defaults the c parameter" do
				expect parse(default_json).c |> to_eq %Complex{ real: 1.0, imag: 0.0 }
			end
			it "defaults the z parameter" do
				expect parse(default_json).z |> to_eq %Complex{ real: 0.0, imag: 0.0 }
			end
			it "defaults the r parameter" do
				expect parse(default_json).r |> to_eq %Complex{ real: 0.0, imag: 0.0 }
			end
			it "defaults the p parameter" do
				expect parse(default_json).p |> to_eq %Complex{ real: 0.0, imag: 0.0 }
			end
		end
	end
