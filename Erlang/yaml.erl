% http://code.google.com/p/yaml-erlang/source/browse/trunk/yaml.erl
...
parse_file(Filename) -> parse(load_file(Filename)).
...
load_file_line(eof, _Infile, Acc) ->
	lists:reverse(lists:map(fun strip_newline/1, Acc));
load_file_line(Line, Infile, Acc) ->
	load_file_line(io:get_line(Infile,''), Infile, [Line|Acc]).

strip_newline(Line) -> [StrippedLine] = string:tokens(Line, "\n"), StrippedLine.
